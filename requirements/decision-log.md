...
## 2026-08-07 Grounding Threshold Recalibration: Not Adjusting The Numbers, Revisit After Chunking

**Decision**: Leave the current grounding thresholds (0.35 grounded / 0.55 partial) unchanged, despite confirmed real drift at the upper boundary. Do not chase a new number now. Revisit only once passage chunking exists (a separate, already-tracked roadmap item), which is the actual prerequisite for a real fix, not another threshold adjustment.

**Why**: Measured real Voyage-embedding distances (not the stub) for 11 hand-picked test queries spanning grounded/ambiguous/ungrounded intent across both jurisdictions, run directly against the current corpus (2026-08-07). Two findings came out of that measurement, together, not either alone:

1. The lower threshold (0.35) is working correctly — every genuinely-grounded test query scored well under it (max 0.32). The upper threshold (0.55) is not: none of the deliberately off-topic test queries cleared it: they landed at 0.41–0.54, in partial territory, reproducing and confirming the original drift finding (0.4548) rather than it being a one-off.
2. Critically, the "ambiguous" and "ungrounded" test categories don't cleanly separate in the real distance numbers at all — in the UK data, a deliberately off-topic question (hive-building wood, 0.4078) scored *closer* to real source material than a genuinely-related-but-unanswered question (healthy queen, 0.4719), the opposite of what a single clean threshold would need to be true. This means moving the 0.55 number to some other value would not cleanly fix the underlying problem — there may be no single number that separates these tiers correctly at the current corpus's resolution.

The root cause both findings point at is structural, not numeric: each Corpus Document is exactly one Passage today (no chunking), so a whole document's semantic content — everything it covers — collapses into a single distance measurement against the query. That's inherently coarse: a document-length passage sits at one point in embedding space regardless of which part of the document a query is actually about, which limits how precisely distance alone can separate "genuinely relevant" from "loosely related" from "irrelevant." Passage chunking (smaller, more topically-focused passages) is the thing that would actually sharpen this resolution; adjusting the threshold number without it would be tuning a symptom while leaving the actual cause — coarse passage granularity — untouched. This also connects to a genuine, separate product question surfaced during the same discussion (whether an off-topic question landing in visibly-caveated `partial` rather than `ungrounded` is actually a problem, given `partial`'s UI already signals lower confidence) — worth keeping in mind when chunking eventually makes a real threshold decision possible again, since the "right" threshold then may depend on how that product question gets answered, not just on the numbers.

**Resolved via**: real-embedding measurement script (11 queries, both jurisdictions, run against the live corpus) plus discussion of three explicit options (leave thresholds as-is trusting the `partial` label; lower the threshold anyway accepting an imperfect fix; defer further threshold work until chunking exists) — user selected the third option directly.

## 2026-08-07 Passage Chunking Mechanism

**Decision**: Two resolutions, covering Slice 0012:

1. Chunking is proven against one deliberately-curated new multi-topic document (a seasonal Varroa IPM guide, split into monitoring / treatment-selection / seasonal-timing Passages) rather than built as speculative infrastructure with no real content to test it against, and rather than dropping the threshold-recalibration link entirely.
2. Passage chunks are authored by the Corpus Curator by hand (one text chunk per `--text`/`--text-file` CLI argument, each individually AI-reviewed), not produced by an automatic paragraph- or token-based splitter.

**Why**: Point 1 was forced by a real finding, not a preference — every existing seeded document is already a single short paragraph with nothing to split, so "chunk today's documents" as the roadmap literally described it would not have touched the measured threshold-drift problem at all. Proving the mechanism needs real multi-topic material to split, or it's just schema plumbing with nothing to demonstrate it works. Point 2 follows this project's existing curated, reviewed-ingestion philosophy (Slice 0006) — the Curator already hand-writes every Passage's text today, and the schema's previously-unused `passages.position` column suggests multi-Passage-per-Document was the originally intended shape. An automatic splitter would be new, unreviewed automated-ingestion machinery this project doesn't otherwise have, for a corpus that is still hand-curated one document at a time; nothing demonstrated a need for it.

**Resolved via**: grilling session with Claude (Challenge mode on point 1, given the real contradiction between the roadmap's framing and the actual corpus content; Choice mode on point 2), during vertical slice planning for Slice 0012.

## 2026-08-07 Grounding Threshold Recalibration Revisit: Chunking Did Not Fix Separation — Working Theory Retracted

**Decision**: Leave the grounding thresholds (0.35/0.55) unchanged, same as the 2026-08-07 decision above. But retract that decision's stated reasoning for *why* a fix was expected once chunking existed — the measured result shows chunking does not do what that entry predicted, and there is currently no known fix, not just a deferred one.

**Why**: Re-ran the same real-Voyage measurement methodology (paced, cached, 21s between calls) against the grown UK corpus, now including the three new chunked Passages from Slice 0012's seasonal IPM guide. Two findings:

1. Chunking works exactly as designed for its proven purpose: all three sub-topic-targeted test queries (monitoring, treatment-selection, seasonal-timing) retrieved precisely the matching chunk as closest match (0.23–0.28), confirming `CorpusRepository`'s retrieval precision claim from Slice 0012 holds under real embeddings, not just the stub.
2. The off-topic/ambiguous separation problem this was meant to help with got *worse*, not better. The same three off-topic UK queries from the prior measurement all moved closer to the corpus after chunking landed: "healthy queen" 0.4719 → 0.4164, "beehive colour" 0.4548 → 0.3888, "hive-building wood" 0.4078 → 0.3994. Nothing crossed 0.35 into a false `grounded` classification, so current behaviour is still safe — but the prior entry's working theory (smaller, more topically-focused Passages would "sharpen resolution" and separate relevance tiers more cleanly) is now empirically contradicted, not just unconfirmed.

The likely real mechanism: the new document that proved chunking's retrieval precision was itself general seasonal-overview content. Splitting it into three chunks narrowed *each individual Passage's* topical scope, but the corpus as a whole still gained three new Passages sitting at moderate distance from any beekeeping-adjacent query — increasing topic density near the ambiguous/ungrounded boundary rather than sharpening it. Finer Passage granularity and broader topical coverage are two different, partly opposed levers; this measurement only tested one document's worth of the second lever, not a systematic test of whether narrower documents would behave differently, so this is not a conclusive proof chunking can never help — only that it did not help here, and the original reasoning for expecting it to was wrong.

**Resolved via**: real-embedding measurement script re-run against the post-chunking corpus (14 queries: the original 11 plus 3 new sub-topic-precision queries, both jurisdictions) — user chose to leave thresholds unchanged and correct the record rather than investigate further or pivot to the product-framing question, since nothing is currently broken and further investigation has no specific next hypothesis backing it yet.

## 2026-08-07 Guest Access And Rate Limiting

**Decision**: Four resolutions, covering Slice 0013:

1. Unauthenticated (guest) users are allowed to submit Varroa queries — the product needs frictionless trial for a prospect trying it — but every guest query is rate-limited, since it triggers real paid Voyage/Claude calls with no accountability for an anonymous caller.
2. The rate limit is keyed by IP address, starting at 10 queries/hour, enforced by a new in-memory `RateLimiter` (not a database- or Redis-backed one).
3. A guest's query resolves to a single well-known, shared Guest Workspace rather than an ephemeral per-visitor Workspace or a new no-Workspace path — keeping `CONTEXT.md`'s "Workspace is the ownership boundary for queries/answers/corrections" invariant intact.
4. This is split from real Google OIDC sign-in, which remains its own future slice with its own grilling pass (OAuth callback, session storage, frontend sign-in UI) — this slice is fully buildable and demoable without any of that.

**Why**: Point 1 is a genuine product/cost trade-off the user raised directly (not a mechanical default) — requiring sign-in for every query would remove the exact frictionless-trial motion that likely matters most right now, but an unauthenticated, unthrottled endpoint has real, unbounded cost exposure once deployed publicly (Fly.io is still on the roadmap). Point 2's IP key and 10/hour starting number were both explicit trade-off choices (session-cookie keying would be more accurate per-visitor but is new client+server session infrastructure that doesn't exist yet; the limit size balances a genuine prospect's single-sitting exploration against bot/script cost risk) — in-memory rather than Redis-backed matches "don't build ahead of demonstrated need," since the near-term deployment plan is single-instance. Point 3 was forced by reading the actual code: `POST /queries` already required `workspace_id` and an active Workspace Membership for every request, with no "no Workspace" path in the domain model at all — a guest has no login and therefore no Membership, so this had to be resolved explicitly, not defaulted. Point 4 avoids rushing a much larger, separate design surface (real OAuth, sessions, frontend sign-in) into the same pass as a narrower, already-buildable cost-control fix.

**Resolved via**: grilling session with Claude (an initial Choice-mode question on guest access itself, prompted directly by the user's own cost concern; a second round of four Choice-mode questions — guest Workspace shape, rate-limit key, exceeded-limit behaviour, limit size — after reading the code surfaced the Workspace-model gap; a final Choice-mode question on splitting from OIDC), during vertical slice planning for Slice 0013.

## 2026-08-07 Google OIDC Sign-In

**Decision**: Five resolutions, covering Slice 0014:

1. Sign-in uses Google Identity Services (GIS), a browser-only JS SDK, not a traditional server-side OAuth Authorization Code+PKCE flow with a backend callback route.
2. The signed-in session is carried as a Bearer token on each request (the Google ID token itself, resent by the frontend), not a cookie.
3. No custom session minting — the backend verifies Google's own ID token directly (~1hr validity, silently refreshed by GIS in the browser) rather than issuing its own longer-lived session JWT.
4. `POST /corrections` now requires real sign-in; the Guest identity (Slice 0013) can no longer flag a Correction.
5. Implementation-time addendum: rejected adding a test-only auth bypass to the production app (a shared-secret env var letting the acceptance suite fake a signed-in session) in favour of accepting a real reduction in browser-level test coverage — the two pre-existing correction-submission scenarios (Slice 0005) were replaced with one scenario proving the sign-in gate; full submission behaviour stays covered at the pytest/`TestClient` seam instead.

**Why**: Point 1 avoids unused complexity — Advisor only needs to know who the user is, never calls a Google API on their behalf, so a server-side redirect flow buys nothing. Point 2 was forced by the current CORS setup: `allow_credentials` isn't set, and cross-origin cookies need that plus `SameSite=None; Secure`, which doesn't even work over plain `http://localhost` without extra dev-only workarounds — a header matches the existing dev-header precedent and needs zero new CORS config. Point 3 avoids new signing-key management and token-issuing code for a UX difference (avoiding GIS's ~1hr refresh cadence) that isn't a demonstrated problem. Point 4 follows directly from FR-007: a Correction is trusted as evidence with no review gate, so attaching a real, accountable identity to that trust is a natural strengthening now that one exists, and it's a lower-friction ask than the query itself. Point 5 was a direct, explicit trade-off: a bypass mechanism is a real, if narrow, security-relevant code path added to the production app purely for test convenience, which the user rejected outright in favour of the coverage move — the same "right seam for the claim" reasoning already established in Slices 0012 and 0013, applied to a case where the honest answer costs some coverage rather than just requiring a different test seam.

**Resolved via**: grilling session with Claude (four Choice-mode questions — sign-in mechanism, session transport, session lifetime, Corrections+auth — during vertical slice planning; a fifth Choice-mode question on the test-bypass trade-off, raised mid-implementation once the real testability conflict was discovered) for Slice 0014.
