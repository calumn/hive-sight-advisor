...
**Mechanism note (2026-08-07, Slice 0014)**: `POST /corrections` now requires a real, verified Google sign-in (Bearer token) — the dev-header mechanism is gone from this route entirely. `POST /queries` gained an *optional* Bearer token: signed-in callers resolve to their own Workspace and skip the guest rate limit, absent-token callers behave exactly as Slice 0013's Guest path. Full correction-submission behaviour (success, duplicate, wrong-workspace, missing/invalid auth) is proven at the `POST /corrections` pytest/`TestClient` seam — Google's real sign-in can't be automated in Playwright without either driving Google's actual consent screen or adding a test-only auth bypass to the app, both rejected. The browser-level suite proves only the sign-in *gate* itself (see FR-007's row below); real end-to-end sign-in is confirmed by a manual live-browser pass per slice, same as every other slice's real-AI-behaviour caveat.
...
| FR-007 | Beekeeper can flag an Answer as wrong; retained as evaluation evidence | `corrections/user-corrections.feature` — *Beekeeper who is not signed in cannot flag an Answer anonymously* (browser-level, proves the sign-in gate). Full submission behaviour (success, duplicate, wrong-workspace, missing/invalid auth) proven at `test_correction_submission_slice.py` (pytest/`TestClient`) — not browser-level, per the Slice 0014 mechanism note above | ✅ Covered |
...
**Second real run found a real generation bug (2026-08-07, fixed same day).** A live-browser pass for Slice 0014 (the first real `/queries` call against the corpus since it grew via Slices 0007/0012) hit `ClaudeGenerationProvider`'s `max_tokens=1024` truncating a genuine multi-passage comparative answer mid-JSON. Fixed: raised to 4096, and added a `TruncatedGenerationError` fail-fast check on `stop_reason` before parsing, so a future recurrence is unambiguous rather than a cryptic `JSONDecodeError`. Covered by new unit tests (`test_generation_claude.py`) using an injectable fake client — the happy path and the truncation path, no live API calls needed.
